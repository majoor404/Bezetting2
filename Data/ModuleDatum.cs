...
namespace Bezetting2
{
    public class ModuleDatum
    {
        ...
        {
            string dienst_ = GetDienst(ProgData.GekozenRooster(), datum, "Blauw");
            if (dienst == dienst_)
                return "Blauw";
            dienst_ = GetDienst(ProgData.GekozenRooster(), datum, "Geel");
            if (dienst == dienst_)
                return "Geel";
            dienst_ = GetDienst(ProgData.GekozenRooster(), datum, "Wit");
            if (dienst == dienst_)
                return "Wit";
            dienst_ = GetDienst(ProgData.GekozenRooster(), datum, "Groen");
            if (dienst == dienst_)
                return "Groen";
            return "Rood";
        }
